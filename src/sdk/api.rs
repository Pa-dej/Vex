...
impl CommandRegistry {
        ...
        F: Fn(CommandSender, Vec<String>) + Send + Sync + 'static,
    {
        ...
        self.inner.commands.insert(
            command_key,
            RegisteredCommand {
                plugin: Arc::from(plugin.to_string()),
                description: Arc::from(description.to_string()),
    ...
    pub fn execute(&self, name: &str, sender: CommandSender, args: Vec<String>) -> bool {
        ...
        if let CommandSender::Player(player) = &sender
            && let Ok(guard) = self.inner.permission_checker.read()
            && let Some(checker) = guard.as_ref()
        {
...
mod tests {
    ...
    use super::{CommandRegistry, CommandSender, MetricsHandle};
