...
/// Metadata operations available for a proxied player.
pub trait PlayerMetaOps {
    /// Sets a typed value under `key`.
    fn set<T>(&self, key: &str, value: T)
    where
        T: Clone + Send + Sync + 'static;

    /// Gets a typed value from `key`.
    fn get<T>(&self, key: &str) -> Option<T>
    where
        T: Clone + Send + Sync + 'static;

    /// Removes an entry from the metadata store.
...
/// Default in-memory per-player metadata store.
...
impl PlayerMeta {
    ...
    where
        T: Clone + Send + Sync + 'static,
    {
        <Self as PlayerMetaOps>::set(self, key, value);
    }
    ...
    where
        T: Clone + Send + Sync + 'static,
    {
        <Self as PlayerMetaOps>::get(self, key)
    }

    /// Removes an entry from the metadata store.
    pub fn remove(&self, key: &str) {
        <Self as PlayerMetaOps>::remove(self, key);
    }

    /// Returns true when `key` exists.
    pub fn has(&self, key: &str) -> bool {
        <Self as PlayerMetaOps>::has(self, key)
    }
}

impl PlayerMetaOps for PlayerMeta {
    fn set<T>(&self, key: &str, value: T)
    where
        T: Clone + Send + Sync + 'static,
    {
